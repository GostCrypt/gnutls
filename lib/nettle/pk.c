	...
	case GNUTLS_PK_GOST_12_512:
		{
			...
			me = _gnutls_gost_q_to_hash(pk_params,
						    &hash_len);
			...
			gostdsa_sign(&priv, NULL, rnd_tmpkey_func, hash_len,
				   vdata->data, &sig);
	...
	case GNUTLS_PK_GOST_12_512:
		{
			...
			_gnutls_gost_q_to_hash(pk_params, &hash_len);

			if (hash_len > vdata->size)
				hash_len = vdata->size;

			ret =
			    gostdsa_verify(&pub, hash_len, vdata->data,
					 &sig);
			if (ret == 0) {
				gnutls_assert();
				ret = GNUTLS_E_PK_SIG_VERIFY_FAILED;
