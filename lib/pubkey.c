					   ...
					   hash, unsigned int *mand)
{
	...
	case GNUTLS_PK_GOST_12_256:
	case GNUTLS_PK_GOST_12_512:
		me = _gnutls_gost_q_to_hash(&key->params, NULL);
		if (hash)
			*hash = (gnutls_digest_algorithm_t)me->id;
 ...
 **/
int
gnutls_pubkey_export_gost_raw(gnutls_pubkey_t key,
			      gnutls_ecc_curve_t * curve,
			      gnutls_digest_algorithm_t * digest,
			      gnutls_gost_paramset_t * paramset,
			      gnutls_datum_t * x, gnutls_datum_t * y)
{
	return gnutls_pubkey_export_gost_raw2(key, curve, digest,
					      paramset, x, y, 0);
}
				       ...
				       gnutls_sign_algorithm_t sign)
{
	...
	if (pubkey->params.algo == GNUTLS_PK_DSA) {
		...
		if (_gnutls_version_has_selectable_sighash(ver)
			...
			_gnutls_gost_q_to_hash(&pubkey->params,
					       &hash_size);
